export default {
    ...
    'sidebar.fallback.label': 'Не распознано',
    ...
    'sidebar.step.desc': 'Ждёт ответ → сохраняет в поле',
    ...
    'props.promptText': 'Реакция на ответ',
    'props.promptHelp':
        'Необязательно. Отправится ПОСЛЕ ответа пользователя, можно использовать {{переменную}}. Сам вопрос задайте в блоке перед шагом.',
    ...
    'export.unconnected':
        'Не подключено к флоу: {blocks}. В сгенерированном проекте эти блоки не создадутся — CLI собирает обработчики только для блоков, на которые ведёт связь или кнопка с переходом. Подключите их, если они нужны боту.',
    'export.warningsTitle': 'Как поведёт себя бот',
    'export.copyJson': 'Копировать JSON',
    'export.download': 'Скачать flow.json',
    'export.copied': 'Скопировано в буфер обмена',
    'toast.exported': 'Файл flow.json скачан. Проект также автосохранён в браузере.',
    'export.nodes': 'узлов',
    ...
    'export.step1': 'Установите Node.js (версия 22 или выше)',
    ...
    'export.afterCommand':
        'Дальше откройте папку my-bot: впишите токены платформ в файл .env и следуйте инструкциям в README.md (установка, запуск, подключение платформ).',
    'export.tip':
    ...
    'help.whatIs': 'Что это?',
    'help.whatIsDesc':
        'Umbot Flow — визуальный редактор для создания ботов на базе фреймворка umbot. Вы создаёте бота, расставляя блоки на холсте и соединяя их. Затем экспортируете конфигурацию и автоматически генерируете готовый проект с кодом.',
    ...
    'help.stepDesc':
        'Ждёт ответ пользователя и сохраняет его в поле, затем ведёт диалог дальше. Вопрос задаёт блок ПЕРЕД шагом (команда или ответ), а текст шага — реакция на ответ. Пример: Ответ «Как тебя зовут?» → Шаг (сохранить в userName) → Ответ «Привет, {{userName}}!».',
    'help.condDesc':
    ...
    'help.close': 'Понятно',
    'help.docsLink': 'Полная документация',
    ...
    'node.preview.noPrompt': '(ждёт ответ)',
    ...
    'palette.navHint': '↑↓ навигация',
    'palette.selectHint': 'Enter выбрать',
    'palette.closeHint': 'Esc закрыть',
    'props.cardHelp':
    ...
    'db.tokensHint':
        'Токены попадут в файл .env сгенерированного проекта. Можно оставить поля пустыми и вписать токены в .env после генерации — так они не окажутся в JSON. Не публикуйте JSON с токенами!',
    ...
    'toolbar.themeLight': 'Переключить на светлую тему',
    'toolbar.themeDark': 'Переключить на тёмную тему',
    ...
    'preview.emptyReply': '(бот ничего не ответил)',
    'preview.defaultFallback': 'Извините, я вас не понял.',
    'preview.blockLimit':
        '[превью остановлено: за один ход выполнено слишком много блоков — проверьте, что выбор пользователя идёт через Шаг]',
    ...
    'validation.v.requiredField': 'обязательное поле',
    'validation.and': 'и',
    ...
    'validation.v.blockCycle':
        'Цикл без шага: {path}. Ответы, действия и условия выполняются сразу, без ожидания пользователя — такой цикл никогда не остановится (CLI откажется генерировать бота). Разорвите цикл Шагом.',
    'validation.w.multipleNext':
        'Из «{name}» выходит связей: {count}. Бот выполнит их все сразу, одним сообщением. Чтобы пользователь выбрал вариант, после блока поставьте Шаг, а ветки разведите Условиями.',
    'validation.w.buttonsWithoutText':
        'У «{name}» есть кнопки, но нет текста ответа: Telegram не отправит такое сообщение (кнопки пропадут), Алиса получит пустой ответ. Добавьте текст.',
    'validation.w.commandTextHidden':
        'Текст команды «{name}» не будет отправлен: связанный блок «{block}» задаёт свой текст (так генерирует CLI). Перенесите текст в «{block}».',
    ...
    'settings.modeHelp':
        'dev — режим разработки с подробными логами.\nprod — продакшен-режим.\nstrict_prod — как prod, но небезопасные регулярные выражения (риск ReDoS) в слотах отклоняются, а не только логируются. Рекомендуется для продакшена.\nРежим попадает в сгенерированный проект (bot.setAppMode).',
